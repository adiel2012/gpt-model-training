...
Eliminates the reward model entirely [rafailov2023direct] (formula \S\ref*{form:dpo}, code Listing~\ref*{lst:dpo}). 90--95\% of RLHF performance at 40--60\% less compute.

> **DPO Successors**
>
> - **SimPO [meng2024simpo**:] Eliminates the reference model entirely. It uses length-normalized log probabilities as an implicit reward and introduces a target reward margin $\gamma$ (see formulation \S\ref*{form:simpo}, code Listing~\ref*{lst:simpo}).
>   - **KTO (Kahneman-Tversky Optimization) [ethayarajh2024kto**:] Operates on binary thumbs-up/down signals rather than preferences. It maximizes the utility of model outputs by aligning with human loss aversion (see formulation \S\ref*{form:kto}, code Listing~\ref*{lst:kto}).
...
DeepSeek's dominant algorithm for reasoning [guo2025deepseekr1] (formula \S\ref*{form:grpo}, code Listing~\ref*{lst:grpo}). Generates a group of 8--64 responses per prompt. Advantages normalized against group statistics. Eliminates the critic model, yielding 33--50\% memory savings over PPO.
...
Removes reference model dependency from GRPO [yu2025dapo] (formulation \S\ref*{form:dapo}, code Listing~\ref*{lst:dapo}). The most streamlined RL alignment approach. Also introduces *clip-higher* (asymmetric policy clipping) and token-level loss normalization to stabilize training.
...
Verifiable rewards for math and code. Binary, objective signal resists reward hacking. DeepSeek R1 demonstrated emergent reasoning from pure RLVR + GRPO without SFT (formulation \S\ref*{form:rlvr}, code Listing~\ref*{lst:rlvr}). The key insight: verifiable correctness is a cleaner training signal than a learned reward model.
...
\toprule
\rowcolor{tablehead}\textcolor{white}{**Stage**} & \textcolor{white}{**Technique**} & \textcolor{white}{**What It Solves**} \\
\midrule
