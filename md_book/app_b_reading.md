# Recommended Reading

  - **Attention Is All You Need** — Ashish Vaswani et al. (2017)
  - **Training Language Models to Follow Instructions with Human Feedback** — Long Ouyang et al. (2022)
  - **Direct Preference Optimization: Your Language Model Is Secretly a Reward Model** — Rafael Rafailov et al. (2023)
  - **Outrageously Large Neural Networks: The Sparsely-Gated Mixture-of-Experts Layer** — Noam Shazeer et al. (2017)
  - **DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via Reinforcement Learning** — Daya Guo et al. (2025)
  - **Switch Transformers: Scaling to Trillion Parameter Models with Simple and Efficient Sparsity** — William Fedus et al. (2022)
  - **LoRA: Low-Rank Adaptation of Large Language Models** — Edward J. Hu et al. (2022)
  - **Constitutional AI: Harmlessness from AI Feedback** — Yuntao Bai et al. (2022)
  - **Training Compute-Optimal Large Language Models** — Jordan Hoffmann et al. (2022)
  - **Chain-of-Thought Prompting Elicits Reasoning in Large Language Models** — Jason Wei et al. (2022)
  - **Editing Models with Task Arithmetic** — Gabriel Ilharco et al. (2023)
  - **Scaling LLM Test-Time Compute Optimally can be More Effective than Scaling Model Parameters** — Charlie Snell et al. (2024)
  - **Llama 2: Open Foundation and Fine-Tuned Chat Models** — Hugo Touvron et al. (2023)
  - **Gemini: A Family of Highly Capable Multimodal Models** — Rohan Anil et al. (2023)
  - **FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning** — Tri Dao (2024)
  - **AI Engineering: Building Applications with Foundation Models** — Chip Huyen (2025)
  - *NLP with Transformers* (2025 rev. ed.) -- O'Reilly -- Hugging Face authors



---

[← Previous Chapter](app_a_glossary.md) | [Table of Contents](../README.md#table-of-contents) | [Next Chapter →](app_c_hardware.md)