...
\begin{lstlisting}[style=pythonstyle, caption={Mixture of Experts (MoE) with top-k routing}, label=lst:moe]
class MoE(nn.Module):
...
\begin{lstlisting}[style=pythonstyle, caption={Lion update step}, label=lst:lion]
...
\subsection{DAPO (Decoupled Alignment for Preference Optimization)}
\label{form:dapo}

Decouples the reference model from the gradient flow to stabilize training and reduce memory overhead.

\begin{lstlisting}[style=pythonstyle, caption={DAPO loss function}, label=lst:dapo]
...
\subsection{RLVR (Reinforcement Learning with Verifiable Rewards)}
\label{form:rlvr}

RL training where rewards are computed by rule-based verifiers (e.g., compilers or unit tests).

\begin{lstlisting}[style=pythonstyle, caption={RLVR update step}, label=lst:rlvr]
